...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function openProject(index) {
        ...
        container.addEventListener('click', e => {
            e.stopPropagation();
            if (e.target.matches('.lb-gallery img')) {
                const viewer = document.getElementById('img-zoom-viewer');
                const target = document.getElementById('img-zoom-target');
                if (viewer && target) {
                    target.src = e.target.src;
                    target.alt = e.target.alt;
    ...
    const hashMatch = window.location.hash.match(/^#projet-(\d+)$/);
    if (hashMatch) {
        const idx = parseInt(hashMatch[1], 10);
        if (idx >= 0 && idx < boxes.length) openProject(idx);
    }
